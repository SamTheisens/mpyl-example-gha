...
def execute_step(proj: Project, stage: str, dry_run: bool = True) -> StepResult:
    ...
    with Repository(RepoConfig.from_config(config)) as repo:
        run_properties = RunProperties.for_local_run(
            config=config, revision=repo.get_sha, branch=repo.get_branch, tag=None, stages=properties["stages"]
        )
...
def find_projects(stage: str) -> list[DynamicOutput[Project]]:
    ...
    invalidated = find_invalidated_projects_for_stage(
        all_projects, stage, changes_in_branch, StepsCollection(get_dagster_logger())
    )
    return list(
        map(
